...
> Status: **v0.8.0; renamed from refract, and back in `v0.x` on purpose.**
> This library was `github.com/timzifer/refract`, froze its API at `v1.0.0` and
> shipped seven minor releases under it. The move to
> `github.com/timzifer/figure` drops the old path's tags, and the version
> restarts at `v0.8.0` so that the three seams
> [ADR 0056](docs/adr/0056-three-dimensional-charts.md) has to widen can be
> widened without a major version and an import-path suffix
> ([ADR 0059](docs/adr/0059-renaming-and-restarting-the-version.md)). Every
> milestone through the old `v1.7.0` is in this tree; see
> [§14](#14-roadmap--milestones) and the [README](README.md) for what each one
> added. `v1.0.0` under this name returns once those seams are widened.
...
- More stats: contour. Normal QQ plots shipped in v1.5, with a general
  ...
  Lorenz — none of which needs a shape figure does not already draw. The
...
- **A major version is a tool, not a failure.** Within a major version the
  ...
  ([ADR 0056](docs/adr/0056-three-dimensional-charts.md)), and the freeze
  returns at `v1.0.0` under this name.
...
3. ~~**Spec fidelity to Vega-Lite**~~ — **settled:** neither a strict subset nor
   "inspired by", but Vega-Lite's *vocabulary* with figure's semantics. The
   names and the structure are Vega-Lite's wherever the concept exists in both;
   what figure has and Vega-Lite does not is named plainly rather than
   smuggled through a borrowed name; and `$schema` says which dialect the
   document is. What is guaranteed is the round trip through figure, tested
   per mark and per scale.
   → [ADR 0014](docs/adr/0014-json-spec.md)
4. ~~**Text ownership**~~ — **settled:** the backend shapes, figure places, and
...
5. ~~**Import path / module org**~~ — **settled:**
   `github.com/timzifer/figure`, with the gg backend at
