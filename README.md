...
> **Status: v0.8.0, and the repository has just been renamed.** This library
> was `github.com/timzifer/refract` and is now `github.com/timzifer/figure`.
> The code is unchanged by the move — it is the tree that shipped as refract
> `v1.7.0` — but the import path is new, the old path's tags do not carry over,
> and the version restarts in the `v0.x` series on purpose
> ([ADR 0059](docs/adr/0059-renaming-and-restarting-the-version.md)).
>
> **The `v0.x` series is deliberate, and it is temporary.** refract froze its
> API at `v1.0.0` and kept that promise through seven minor releases. Three
> seams in it are the wrong shape for a third dimension
> ([ADR 0056](docs/adr/0056-three-dimensional-charts.md)), and correcting them
> under the old name meant a `v2` and an import-path suffix for a library with
> no users to migrate. The rename makes the correction free, and `v0.x` says
> plainly that it has not happened yet. A `v1.0.0` under this name comes back
> once those seams are widened.
>
> Everything the old name shipped is here: every milestone through **v1.0** and
> the seven after it — a null that is a missing value in a text or temporal
> column, a tick format and a language a *document* can choose, an interval
> mark, a second axis in either direction, and a PDF that carries the font its
> labels need; `geom.Treemap`, `geom.Icicle`, `geom.Sankey` and `geom.Arc`,
> which are also a sunburst and a chord diagram once the coordinate stage has
> had them ([ADR 0039](docs/adr/0039-relational-layouts.md));
> `geom.AvoidOverlap` for panel-local label placement
...
> colour ramps that compress their domain (`scale.ColorLog`) or cut it into
> classes (`scale.Threshold`, `scale.Quantize`, `scale.Quantile`)
> ([ADR 0042](docs/adr/0042-colour-transforms-and-classes.md)); and
> **identity, transitions, and guides that answer to a pointer** — `geom.KeyBy`
> names the column that says which row is which
> ([ADR 0043](docs/adr/0043-mark-identity.md),
> [ADR 0044](docs/adr/0044-transitions.md)); the host wires one chart to another
> and figure supplies the two ends of the wire
...
>
> The nested modules are tagged with the core at `v0.8.0`: `backend/gg` and
> `backend/window` share the core's version, the opt-in GPU tier is at
> `v0.3.0`, and the Arrow adapter at `arrow/v18.0.4`, whose major is Arrow's.
