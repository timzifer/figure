...
// The allocation gate.
//
// It is built out under the race detector, which allocates on figure's
...
// makes, into a backend that does nothing — so what is measured is figure's
...
func allocsPerFrame(t *testing.T, p *figure.Plot) float64 {
	t.Helper()
...
// The allocation gate.
//
// The claim figure makes is not that a render allocates nothing — a frame
