...
// without any coordinate in figure changing.
...
func (t *target) Open(s ir.Surface) (ir.Backend, error) {
	...
	if !ctx.Truthy() {
		return nil, errors.New("figure/backend/canvas: no 2D context")
	}
	t.b = &backend{
	...
	if !t.b.path2d.Truthy() {
		return nil, errors.New("figure/backend/canvas: this environment has no Path2D")
	}
	t.b.begin()
	return t.b, nil
}
...
//
// This is the seam ADR 0003 describes: the backend shapes and figure places,
