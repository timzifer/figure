...
// needed one would be a test nobody could run. What matters is that figure
...
func linePlot() *figure.Plot {
	src := figure.Float64Columns(map[string][]float64{
		"x": {0, 1, 2, 3},
		"y": {0, 2, 1, 3},
	})
	p := figure.New(figure.Size(400, 300), figure.Title("Canvas"))
...
func TestMeasureUsesTheContextFont(t *testing.T) {
	...
	// from the context rather than from a table figure carries.
...
func TestElementIsSizedForTheDevicePixelRatio(t *testing.T) {
	...
	// The transform carries the ratio, so no coordinate in figure changes.
