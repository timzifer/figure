...
// through `syscall/js`, which is the standard library, so figure in a browser
// links exactly what figure on a server links.
//
//	//go:build js && wasm
//
...
// path to take and no canvas fallback to fall back to. figure therefore draws
