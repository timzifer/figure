...
func surfacePlot() *figure.Plot {
	src := figure.Float64Columns(map[string][]float64{
		"x": {0, 1, 2, 3},
		"y": {0, 2, 1, 3},
	})
	p := figure.New(figure.Size(200, 150), figure.Title("Surface"))
...
// The raster backend has to honour an italic run, because figure now produces
