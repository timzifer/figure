...
// transparency group. figure's gradients come from colour ramps whose stops
...
func (b *backend) Text(run ir.TextRun) {
	...
	// figure's top-left space. Rotation composes into the same matrix.
...
// textOffset converts figure's two-axis alignment into an offset in text
...
func (b *backend) imageObject(img image.Image) (int, error) {
	...
	if w <= 0 || h <= 0 {
		return 0, errors.New("figure/backend/pdf: empty image")
	}
...
func (b *backend) Pop() {
	if b.depth == 0 {
		b.fail(errors.New("figure/backend/pdf: Pop without matching Push"))
		return
	}
	b.depth--
	b.op("Q")
...
func (b *backend) Flush() error {
	...
	if b.depth != 0 {
		return fmt.Errorf("figure/backend/pdf: %d unclosed graphics state(s) at Flush", b.depth)
	}
	...
	// The page opens by flipping Y, so figure's top-left coordinates are
