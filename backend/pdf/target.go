...
import (
	...
	"github.com/timzifer/figure/internal/sfnt"
...
func WithFont(regular, bold, italic []byte) Option {
	return func(o *options) {
		...
		if err != nil {
			o.fontErr = fmt.Errorf("figure/backend/pdf: parsing the supplied regular font: %w", err)
			return
		}
		if !reg.HasUnicodeMap() {
			o.fontErr = errors.New("figure/backend/pdf: the supplied regular font has no Unicode character map, so no label could be mapped to a glyph")
			return
		}
...
func optionalFont(b []byte, which string) (*sfnt.Font, error) {
	...
	f, err := sfnt.Parse(b)
	if err != nil {
		return nil, fmt.Errorf("figure/backend/pdf: parsing the supplied %s font: %w", which, err)
	}
	return f, nil
}
...
func (t *target) Close() error {
	...
	if t.b != nil && t.bw != nil {
		if t.b.rootRef == 0 {
			err = errors.New("figure/backend/pdf: Close before Flush")
		} else {
