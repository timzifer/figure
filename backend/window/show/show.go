...
// window loop there. Handlers registered with [figure.Plot.On] fire from that
...
// [figure.Responsive] as well and the type and stroke weights follow the size;
...
func Plot(p *figure.Plot, opts ...window.Option) error {
	if p == nil {
		return errors.New("figure/backend/window/show: nil plot")
	}
...
func Into(w *window.Window, p *figure.Plot) error {
	if w == nil || p == nil {
		return errors.New("figure/backend/window/show: nil window or plot")
	}
