...
// take them. See [github.com/timzifer/figure/backend/gg.WithFont].
...
func New(opts ...Option) *Window {
	c := config{title: "figure", w: 800, h: 500, resize: true}
	for _, o := range opts {
		o(&c)
	}
...
func (w *Window) Run(h Handler) error {
	if w.app != nil {
		return errors.New("figure/backend/window: this window has already been run")
	}
...
func (w *Window) upload(dc *gogpu.Context, img image.Image) error {
	...
	if err != nil {
		return fmt.Errorf("figure/backend/window: uploading the chart: %w", err)
	}
	if w.tex != nil {
		w.tex.Destroy()
	}
