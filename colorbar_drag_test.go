...
func dragBar(t *testing.T, p *figure.Plot, live *figure.Live, from, to ir.Point) figure.Event {
	t.Helper()
	var got figure.Event
	p.On(figure.Select, func(ev figure.Event) { got = ev })
	in := live.Input()
...
func continuousBarChart(t *testing.T) (*figure.Plot, *figure.Live) {
	t.Helper()
...
func barExtent(t *testing.T, live *figure.Live) (x, top, bottom float32) {
	t.Helper()
...
func TestDraggingAContinuousBarSelectsARange(t *testing.T) {
	...
	if ev.Kind != figure.Select {
		t.Fatalf("kind = %v, want select", ev.Kind)
	}
	if ev.Hit.Kind != figure.Colorbar {
...
func TestADragOnABarIgnoresTheDragMode(t *testing.T) {
	for _, mode := range []figure.Drag{figure.DragPans, figure.DragSelects, figure.DragZooms} {
		t.Run(mode.String(), func(t *testing.T) {
			...
			if got.Hit.Kind != figure.Colorbar {
...
func TestAClickOnABarIsStillAClick(t *testing.T) {
	...
	p.On(figure.Select, func(figure.Event) { selects++ })
