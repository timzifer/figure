...
func guideChart(t *testing.T, layer geom.Geom) *figure.Live {
	t.Helper()
	p := figure.New(figure.Size(640, 320))
...
func guideSource() figure.Source {
	return figure.Float64Columns(map[string][]float64{
		"x": {0, 1, 2, 3},
		"y": {10, 20, 15, 25},
		"v": {0, 33, 66, 100},
	})
...
func sweep(t *testing.T, l *figure.Live, kind figure.Kind) []figure.Hit {
	t.Helper()
	var out []figure.Hit
...
func TestAContinuousColorbarReportsTheValueUnderThePointer(t *testing.T) {
	...
	hits := sweep(t, live, figure.Colorbar)
	if len(hits) != 1 {
	...
	for y := float32(2); y < 320; y++ {
		...
		if !ok || hit.Kind != figure.Colorbar {
			continue
		}
		if found == 0 {
			top = hit.Value
...
func TestASizeKeyRowReportsItsValue(t *testing.T) {
	...
	hits := sweep(t, live, figure.SizeKey)
	if len(hits) == 0 {
...
func TestNoGuideReportsAPanelValueOrARow(t *testing.T) {
	for _, c := range []struct {
		...
		{"continuous", geom.Scatter(guideSource(), geom.X("x"), geom.Y("y"),
			geom.ColorBy("v", scale.Sequential(palette.Viridis))), figure.Colorbar},
		{"classed", geom.Scatter(guideSource(), geom.X("x"), geom.Y("y"),
			geom.ColorBy("v", scale.Quantize(palette.Viridis, 3))), figure.Colorbar},
		{"size", geom.Scatter(guideSource(), geom.X("x"), geom.Y("y"),
			geom.SizeBy("v", scale.Size(scale.SizeRange(4, 24)))), figure.SizeKey},
	} {
...
func TestAHoverInTheMarginFindsAColorbar(t *testing.T) {
	...
	if !got.Found || got.Hit.Kind != figure.Colorbar {
...
func TestAChartWithoutGuidesIndexesNone(t *testing.T) {
	...
	for _, k := range []figure.Kind{figure.Colorbar, figure.SizeKey, figure.LegendRow} {
...
func TestClickingAColourbarBandReportsItsInterval(t *testing.T) {
	...
	p.On(figure.Click, func(ev figure.Event) {
		if ev.Hit.Kind != figure.Colorbar || ev.Hit.Class < 0 {
			return
		}
	...
	var middle figure.Hit
	for _, h := range sweep(t, live, figure.Colorbar) {
		if h.Class == 1 {
			middle = h
		}
	}
...
func TestClickingASizeKeyRowReportsItsValue(t *testing.T) {
	...
	rows := sweep(t, live, figure.SizeKey)
	if len(rows) == 0 {
		t.Fatal("no size key")
	}
	at := rows[0].At
