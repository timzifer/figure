...
//
// Where figure differs from D3 is what happens next, and it is worth being
...
func (t *Tween) plan() error {
	...
	for _, name := range append(append([]string{}, t.b.Columns()...), t.a.Columns()...) {
		...
		switch {
		case oka && okb && ka != kb:
			return fmt.Errorf("figure/data: column %q is %s in one state and %s in the other", name, ka, kb)
		case !oka && !okb:
			continue
		}
		k := kb
	...
	if len(t.names) == 0 {
		return errors.New("figure/data: the two states share no columns")
	}
	return nil
}
