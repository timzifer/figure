...
> **Current shape:** the adapter answers one `data.Column` call rather than
> three typed ones, and an Arrow `Int64` column arrives as exact integers —
> which is what keeps an identifier past 2^53 from becoming its neighbour
> ([ADR 0061](0061-columns-are-one-value.md)). The borrow described below
> holds for `Float64` and for `Int64`.
...
already the shape figure's data layer wants. `data.Source` is documented to
...
- `github.com/timzifer/figure/arrow` is a nested module with its own `go.mod`,
...
- **A null becomes NaN** in a numeric column, the zero time in a temporal one,
  the empty string in a categorical one. NaN is the value figure's
...
- Importing `figure` still links nothing. Importing `figure/arrow` links
