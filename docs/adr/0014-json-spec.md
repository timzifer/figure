...
**A strict Vega-Lite subset.** figure would emit documents a Vega-Lite renderer
...
placement — and figure has no transforms, no selections, no aggregate
pipelines, no `repeat`, no `vconcat`. Everything figure has that Vega-Lite does
...
**A figure-native format.** Name everything ourselves, round-trip perfectly,
...
**Vega-Lite's vocabulary, figure's semantics.** Borrow the names and the
...
- Where Vega-Lite has the concept, the document uses Vega-Lite's name and
  ...
  figure spec without a manual, which is most of what the compatibility was
  ever worth.
...
- Where figure has something Vega-Lite does not, the property is figure's and
  ...
  it.
- `$schema` is `https://github.com/timzifer/figure/spec/v0.7` (it was `v0.5`
...
**What is guaranteed is the round trip through figure.** `spec.Of` then
...
- The spec covers every geom, every scale, every annotation and every facet
  figure has, because the format was allowed to grow past Vega-Lite where
  figure has.
...
- Colour ramps are now registered by name (`palette.RegisterRamp`), using the
  ...
  the same thing in a figure spec and in a Vega-Lite one.
...
- Reading is lenient where writing is strict. A hand-written document with no
  ...
  figure wrote always carries the parse map, so the inference only ever runs
