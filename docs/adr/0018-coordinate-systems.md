...
| `spec` | a figure-owned top-level `"coord"` field, plus `coord.Desc` / `coord.FromDesc` in the shape of `scale.Desc`. ADR 0014 requires that what figure has and Vega-Lite does not be named plainly rather than smuggled through a borrowed name — so a pie is written as `mark: "bar"` with `coord: {"type": "polar"}`, not as an `arc` mark that would round-trip into something figure cannot rebuild |
