...
2. **Write a second rasterizer** against the window's swapchain. That is a
   second implementation of every mark figure draws, and the day it disagrees
...
- `backend/window` is a **nested module** requiring `gogpu/gogpu` and
  `backend/gg`. Importing figure's core still yields a stdlib-only dependency
...
- `window.Window` is a surface and an event loop, and knows nothing about
  charts. `window/show` is a second package that joins one to a `*figure.Plot`.
...
- The steering itself is `figure.Input`, in the core: a portable state machine
