...
cannot accept a `v19` record, so a figure
...
`/v2` rule — and `github.com/timzifer/figure/arrow` at `v18.0.0` is not a
...
**The adapter lives at `github.com/timzifer/figure/arrow/v18`, in the
...
- The module path carries Arrow's major version, and the package is still
  ...
  `import "github.com/timzifer/figure/arrow/v18"` and uses `arrow.Source`,
...
Arrow-go stops changing its major version with every release, or figure
