...
3. **Hit-testing, parallel rendering and the widget need no new code.**
   ...
   inside one `Plot`, so `fyne-figure`'s widget — which wraps exactly one
...
above a `Plot`, `figure.Link(LinkX, a, b)`. It was rejected as the *primary*
...
- `figure.Grid` has no `Live`. A linked pair would be static, or would need a
  second interaction path.
- `fyne-figure`'s widget holds one Plot. Two linked plots would need a
...
framing. **No `figure.Link` was added**: it would be a name for what passing
