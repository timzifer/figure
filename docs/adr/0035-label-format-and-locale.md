...
type. A grammar of figure's own rather than d3-format's, because the subset
...
— and `figure.Locale` walks the scales the chart description holds and calls
...
**A locale is data, and the ones that ship are the ones figure could get
...
table figure cannot check would be worse than shipping none, because a chart
...
- **The document carries one `format` field, not two.** A numeric scale reads
  ...
  `format` as well, on the axis object figure does not have.
