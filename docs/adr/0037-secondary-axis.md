...
**Status:** Accepted · **Date:** 2026-09-07 · see [Amendment](#amendment-the-horizontal-direction)

> **Current shape:** the scales a layer reads are fields on
> `render.LayerInfo`, which `Observer.Layer` takes, and `coord.Opposite` is
> one method — `FurnitureOpposite(dst, coord.FurnitureRequest)` — with the
> edge read from the request ([ADR 0060](0060-parameter-structs-at-every-seam.md)).
> The reasoning below is unchanged; the signatures are not.

## Context

`figure.Plot` had exactly one X scale and one Y scale, and a layer had no way
...
A scale does not know which marks read it — nothing in figure's scales ever
