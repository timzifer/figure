# 0045 — Linked views are the host's; figure supplies the two ends of the wire
...
**A linking engine.** Charts declare a shared selection; figure propagates it.
That is what a dashboard framework does, and it is a statement about *two*
charts — while figure's entire model, from `Plot` down through `render.Chart`,
...
one of those is state figure does not have and would have to start keeping.
...
**figure supplies identification, selection and the ability to act without
...
- **There is no `Live.Highlight` and no selection state inside figure.** The
...
- The overlay layer bucket H asked for — a tooltip, a crosshair, a brush
  rectangle figure itself draws — is **still not here**. `Input.Dragged`
