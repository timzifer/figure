...
filter, or nothing at all, and figure cannot know which.
...
```go
switch ev.Hit.Kind {
case figure.LegendRow: live.Toggle(ev.Hit.Layer)
case figure.Colorbar:  filter(ev.Hit.Lo, ev.Hit.Hi)
case figure.SizeKey:   pick(ev.Hit.Value)
}
```
...
misleading. `figure.Legend` was already the option that asks a plot for one,
...
with the column in hand — figure would have to guess which of several layers
was meant.
