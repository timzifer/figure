...
> renamed to `github.com/timzifer/figure` and restarted at `v0.8.0`
> ([ADR 0059](0059-renaming-and-restarting-the-version.md)), which makes the
> same break a minor release in a `v0.x` series and leaves the import path
> without a suffix. **Nothing about the design below changes** — the three
...
[ADR 0055](0055-depth-without-a-third-axis.md) split "3D" into three features
...
**Take the break.** Widen the seams that are the wrong shape, ship it in the
`v0.x` series the rename put the library back into, and say so in the release
notes.
...
**3D is `figure/three`, a package of the core module, and the three seams that
...
**A freeze with no subscribers is a habit, not a promise.** The library tagged
`v1.0.0` under its old name and reached `v1.7.0`; the freeze was the right
discipline for getting the model right and it has done its work. What it is not is a reason to bolt four optional
...
| Seam | frozen shape | widened shape |
|---|---|---|
...
> corrected rather than papered over with a parallel path — and while figure
...
argued for; it moves it inside the perimeter, where it is `figure/three`
beside `figure/render` rather than a second path *through* `render`. `three`
...
The import path is `github.com/timzifer/figure/three` either way — a nested
...
[0057](0057-orbiting-a-chart.md) turns it.
...
- `Index.Panel` takes an x scale and a y scale, so `Hit.X` and `Hit.Y` invert
  ...
  it supplied. This is [ADR 0045](0045-linked-views.md)'s bargain again: figure
...
- **No perspective camera.** Orthographic only, for [0055](0055-depth-without-a-third-axis.md)'s
...
- **No 3D pie**, for the reason 0055 gives, which does not weaken by being
  restated.
