...
The third of the features [ADR 0055](0055-depth-without-a-third-axis.md) split
...
without [0056](0056-three-dimensional-charts.md) and everything after it — a
...
[CONCEPT §5](../../CONCEPT.md#5-non-goals) having said twice what figure is
...
scales are figure's. [ADR 0045](0045-linked-views.md) declines to own linked
views — because a link is a statement about two charts, and figure's model is
about one. The rule that produces both: **figure owns what is a statement
...
live, _ := plot3.Live(target)     // mirrors figure.Live's shape
live.Camera(cam)
live.Draw()
...
answer is that a camera is not a hit and the module does not need figure's
event vocabulary to receive two floats. The core is untouched by this record as
completely as it is by 0056.
...
- **No perspective**, still. 0055 refused it for the reading and 0056 kept the
...
- **No orbiting of a 2D chart.** `coord.Oblique`'s depth vector is a constant
  of the chart (0055) and turning it would be animating a decoration. A reader
  who wants to look at data from another angle wants 0056's chart.
