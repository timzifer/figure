...
without `scale.Nice()` is the recipe, and `figure.Coord`'s doc comment says so.
...
| **Voronoi** | **drawn** since [ADR 0080](adr/0080-nearest-neighbour-cells.md) — `geom.Voronoi` over `stat.Voronoi`, and this verdict was right that nothing architectural stood in the way and wrong about what the chart is for: it is a coverage map and a map of the nearest measurement, neither of which is a hit-test question. Not Fortune's sweep but an intersection of half-planes, which is exact, has no degenerate cases and is bounded by what a reader can read; the cells are cut **on the panel**, because equidistance needs a length and two axes need not share one. The hit-test index the row named is still unwritten and still a performance question ([ADR 0015](adr/0015-hit-testing.md)) |
