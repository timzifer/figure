...
func latency(path string) error {
	...
	p := figure.New(
		figure.Size(820, 320),
		figure.Title("Request latency"),
		figure.XTitle("milliseconds"),
		figure.YTitle("requests"),
	)
	p.X(scale.Linear(scale.Nice()))
	p.Y(scale.Linear(scale.Nice(), scale.Zero()))
	p.Add(geom.Histogram(figure.Float64Columns(map[string][]float64{"ms": vs}),
	...
	q := figure.New(
		figure.Size(820, 320),
		figure.Title("Request latency, cumulative"),
		figure.XTitle("milliseconds"),
		figure.YTitle("fraction of requests"),
	)
	q.X(scale.Linear(scale.Nice()))
	q.Y(scale.Linear())
	q.Add(geom.ECDF(figure.Float64Columns(map[string][]float64{"ms": vs}),
		geom.X("ms"), geom.Label("cumulative")))
	return q.Render(figure.SVG(cumulativePath(path)))
}
...
func services(path string) error {
	...
	p := figure.New(
		figure.Size(860, 420),
		figure.Title("Latency by service"),
		figure.YTitle("milliseconds"),
		figure.Legend(true),
	)
	p.X(scale.Ordinal())
	p.Y(scale.Linear(scale.Nice(), scale.Zero()))
	p.Add(geom.Violin(tbl,
		...
		geom.Bandwidth(3)))
	return p.Render(figure.SVG(path))
}
...
func seasons(path string) error {
	...
	p := figure.New(
		figure.Size(760, 560),
		figure.Title("Daily maximum, by month"),
...
func cloud(path string) error {
	...
	p := figure.New(
		figure.Size(820, 500),
		figure.Title("Fifty thousand observations"),
		figure.XTitle("x"),
		figure.YTitle("y"),
		figure.Legend(false),
	)
...
func nations(path string) error {
	...
	p := figure.New(
		figure.Size(860, 520),
		figure.Title("Income and life expectancy"),
		figure.XTitle("income per person"),
		figure.YTitle("years"),
		figure.Legend(true),
	)
