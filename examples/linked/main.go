...
// figure: the library says what the pointer is on, the host decides what that
// means somewhere else, and the second chart is told in the same vocabulary
// any other caller would use.
//
// There is no link between the two charts inside figure, and that is
// deliberate. A link is a statement about two charts, and figure's model is
...
//     hovered stage feeds. Both are drawn by figure, over the finished chart,
...
func run(throughputPath, flowPath string) (hovered string, highlighted int, err error) {
	...
	line := figure.New(
		...
		figure.Size(720, 360),
		figure.Title("Throughput by stage"),
		figure.XTitle("hour"),
	...
	flow := figure.New(
		figure.Theme(bare()),
		figure.Size(720, 360),
		figure.Title("Bytes between stages"),
	)
	...
	// here rather than composed by figure.
	...
	cross := &figure.Crosshair{Panel: -1}
	tip := &figure.Tooltip{}
	line.Overlay(figure.Overlays{cross, tip})

	rings := &figure.Highlight{Panel: -1, Radius: 10}
	flow.Overlay(rings)

	// The wire. A hover in chart 1 arrives here with a key; what that key
	// means in chart 2 is this program's business, and nothing figure could
	// have guessed.
	line.On(figure.Hover, func(ev figure.Event) {
	...
	if err := line.Render(figure.SVG(throughputPath)); err != nil {
		return "", 0, err
	}
	if err := flow.Render(figure.SVG(flowPath)); err != nil {
		return "", 0, err
	}
	return hovered, highlighted, nil
}

// flowsThrough is the mapping figure does not own: which edges touch a stage.
// It is a scan here; in a real program it is whatever answers the question,
// and figure never needs to know. An empty key — the pointer is over nothing,
