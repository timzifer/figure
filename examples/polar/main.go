...
func browserShare(out string) error {
	...
	p := figure.New(
		figure.Size(620, 400),
		figure.Title("Browser share"),
		figure.Theme(bare(theme.Light)),
...
func windRose(out string) error {
	...
	p := figure.New(
		figure.Size(560, 520),
		figure.Title("Hours by wind direction"),
...
func spending(out string) error {
	...
	p := figure.New(
		figure.Size(620, 440),
		figure.Title("Spend by team, against each team's own budget"),
		figure.Theme(bare(theme.Light)),
