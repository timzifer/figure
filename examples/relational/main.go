...
func layout(title string, w, h int, c coord.Coord, opts ...figure.Option) *figure.Plot {
	base := []figure.Option{figure.Size(w, h), figure.Title(title), figure.Theme(bare())}
	if c != nil {
		base = append(base, figure.Coord(c))
	}
	p := figure.New(append(base, opts...)...)
...
func diskTreemap(path string) error {
	p := layout("Disk by directory", 700, 420, nil, figure.Legend(false))
...
func diskIcicle(path string) error {
	p := layout("Disk by depth", 640, 380, nil, figure.Legend(false))
...
func diskSunburst(path string) error {
	p := layout("Disk by directory", 520, 460, coord.Polar(coord.Hole(0.12)), figure.Legend(false))
	p.Add(geom.Icicle(disk(),
		geom.ID("path"), geom.Parent("under"), geom.Value("kb"),
		geom.Padding(0.004)))
	return p.Render(figure.SVG(path))
