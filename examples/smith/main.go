...
// rim.
func smithAxes(p *figure.Plot) *figure.Plot {
	p.X(scale.Linear(
		scale.Domain(0, 50),
...
func matchingNetwork(out string) error {
	...
	p := smithAxes(figure.New(
		figure.Size(620, 560),
		figure.Title("Matching 15 − j25 Ω to 50 Ω"),
		figure.Theme(theme.Light),
		figure.Coord(coord.Smith(coord.SmithArc())),
		figure.Legend(false),
	))
...
func shuntOnTheYChart(out string) error {
	...
	p := smithAxes(figure.New(
		figure.Size(620, 560),
		figure.Title("The same two steps, read as admittance"),
		figure.Theme(theme.Light),
		figure.Coord(coord.Smith(coord.SmithAdmittance(true))),
		figure.Legend(false),
	))
