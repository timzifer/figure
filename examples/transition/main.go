...
//   - **The host owns the clock.** figure reads none. Transition.At(f) is the
...
func run(prefix string) (data.Alignment, error) {
	...
	tr.Ease(figure.EaseInOut)

	// The loop is this program's, not figure's. Advance would do the same
	...
	for _, f := range []struct {
		...
		if err := p.Render(figure.SVG(prefix + "-" + f.name + ".svg")); err != nil {
			return data.Alignment{}, err
		}
	}
