// Package figure turns one declarative chart specification into any output
...
// github.com/timzifer/figure/backend/gg, which is still CGO-free.
//
// # Shape of the API
//
...
import (
	...
	coordpkg "github.com/timzifer/figure/coord"
...
func freeScale(s scale.Scale) (scale.Scale, error) {
	c, ok := s.(scale.Cloner)
	if !ok {
		return nil, fmt.Errorf("figure: %T cannot be given to a free facet axis: it does not implement scale.Cloner", s)
	}
	return c.Clone(), nil
}
