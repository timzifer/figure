...
type Desc struct {
	...
	Marker    ir.Marker
	MarkerSet bool
	// Hatch is the pattern laid over a filled mark, and HatchSet reports
	// whether the layer chose it. The pair is [Desc.Marker] and MarkerSet once
	// more, for the same reason: [ir.HatchNone] is both the zero value and a
...
func (d Desc) options() []Option {
	...
	if d.Corner > 0 {
		opts = append(opts, Corner(d.Corner))
	}
	if d.Inset > 0 {
		opts = append(opts, Inset(d.Inset))
	}
	if d.AlignSet {
