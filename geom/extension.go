...
func Register(m Mark, build func(Desc) (Geom, error)) {
	if m == "" {
		panic("figure/geom: Register: a mark needs a name")
	}
	if build == nil {
		panic("figure/geom: Register: a mark needs a builder")
	}
	if builtin(m) {
		panic(fmt.Sprintf("figure/geom: Register: %q is a built-in mark", m))
	}
	registryMu.Lock()
...
func builtin(m Mark) bool {
	...
	case MarkLine, MarkScatter, MarkBar, MarkArea, MarkStep, MarkBoxplot, MarkRect,
		...
		MarkHLine, MarkVLine, MarkHBand, MarkVBand, MarkSegment, MarkRegion, MarkNote, MarkLocus, MarkText:
		return true
	}
	return false
}
