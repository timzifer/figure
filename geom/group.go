...
// rest of figure already uses: a facet's panels, a categorical axis and a
...
func (gs *groups) train(src data.Source, s series, c config, x, y scale.Scale, stack Stacking) error {
	...
	if len(labels) != len(s.x) {
		return fmt.Errorf("figure/geom: group column %q has %d rows and the plotted columns have %d",
