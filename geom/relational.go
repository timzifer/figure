...
// [Icicle] under [github.com/timzifer/figure/coord.Polar] is a sunburst and an
...
func (t *tree) reset(src data.Source, c config) error {
	...
	for i, id := range ids {
		if t.of(id, i) != i {
			return fmt.Errorf("figure/geom: %q appears twice in %q, and a hierarchy's node names have to be distinct", id, c.idCol)
		}
	}
