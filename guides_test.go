...
func legendChart(t *testing.T, legend bool) *figure.Live {
	t.Helper()
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1, 2}, "y": {1, 2, 3}})
	p := figure.New(figure.Size(600, 300), figure.Legend(legend))
	p.X(scale.Linear())
	p.Y(scale.Linear())
