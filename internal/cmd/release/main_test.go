...
import (
	...
	"github.com/timzifer/figure/internal/release"
)
...
// tags is a repository history: the v0.7.0 release, tagged across all five
// modules the way this command tags them.
var tags = []string{
	"v0.6.0", "v0.7.0",
	"backend/gg/v0.6.0", "backend/gg/v0.7.0",
	"backend/window/v0.7.0",
...
func TestPlanDerivesEveryTagFromTheCoreVersion(t *testing.T) {
	got, err := plan(v("v0.8.0"), "colour transforms", nil, nil, tags)
	if err != nil {
		t.Fatal(err)
	}
	want := map[string]string{
		"v0.8.0":                "v0.8.0 — colour transforms",
		...
		"backend/gg/gpu/v0.2.1": "backend/gg/gpu v0.2.1 — the tier at core v0.8.0",
...
func TestPlanTakesOverridesAndSkips(t *testing.T) {
	...
	if want := "v0.8.0 backend/gg/v0.8.0 backend/gg/gpu/v0.3.0"; strings.Join(names, " ") != want {
...
func TestPlanRefusesWhatCannotBePublished(t *testing.T) {
	for name, run := range map[string]func() error{
		"a core release with no summary": func() error {
			_, err := plan(v("v0.8.0"), "", nil, nil, tags)
			return err
		},
		...
		"an override that is not a version": func() error {
			_, err := plan(v("v0.8.0"), "x", map[string]string{"arrow/v18": "18.0.4"}, nil, tags)
			return err
		},
		"a module that does not exist": func() error {
			_, err := plan(v("v0.8.0"), "x", nil, []string{"backend/pdf"}, tags)
			return err
		},
		"a first release with nothing to move on from": func() error {
			_, err := plan(v("v0.8.0"), "x", nil, nil, []string{"v0.7.0"})
			return err
		},
		"skipping everything": func() error {
			_, err := plan(v("v0.8.0"), "x", nil,
...
func TestPreconditionRefusesATagThatExists(t *testing.T) {
	got, err := plan(v("v0.7.0"), "already out", nil, nil, tags)
	if err != nil {
		t.Fatal(err)
	}
	if err := precondition(got, tags); err == nil {
		t.Error("re-cutting v0.7.0 was accepted")
	}
}
