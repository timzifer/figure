...
func TestTagMessagesFollowTheirModule(t *testing.T) {
	core, _ := Find(".")
	v := Version{0, 8, 0}
	if got, want := core.Message(v, v, "colour transforms"), "v0.8.0 — colour transforms"; got != want {
		t.Errorf("core message %q, want %q", got, want)
	}
	arrow, _ := Find("arrow/v18")
	if got, want := arrow.Message(Version{18, 0, 4}, v, ""), "arrow/v18.0.4 — the adapter at core v0.8.0"; got != want {
		t.Errorf("arrow message %q, want %q", got, want)
	}
	gg, _ := Find("backend/gg")
	if got, want := gg.Message(v, v, ""), "backend/gg v0.8.0 — the raster path at core v0.8.0"; got != want {
