...
func Font(cmapTable []byte, numH int) []byte {
	...
	return assemble(map[string][]byte{
		...
		"OS/2": os2, "post": post, "name": nameTable("Figure Test"),
	})
}
...
// figure interprets charstrings. What such a font exercises is the half that
// is figure's own — that the format is recognised, that it is reported as
