...
// sink: figure lowers a chart into calls on a Backend, in paint order.
//
// # Stability
//
...
// it — [Partial], [Resizer], [Semantics] are the three that exist — and figure
// asks with a type assertion and does without when the answer is no. It exists
// so that figure is insulated from any single rendering library: a change in
...
// [image.Image] — is **lent for the duration of that call**. figure draws from
...
// # Concurrency
//
// A Backend is not safe for concurrent use. figure may build IR on several
...
//
// Splitting Target from Backend is what lets figure.Render take a destination
...
//
// figure calls Describe once, before any drawing call, so that a backend
