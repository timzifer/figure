...
func (p *Plot) Live(t Target) (*Live, error) {
	if t == nil {
		return nil, errors.New("figure: nil render target")
	}
...
func (l *Live) Rescale(dpr float64) error {
	if l.b == nil {
		return errors.New("figure: Rescale on a closed Live")
	}
	if dpr <= 0 {
		return fmt.Errorf("figure: device pixel ratio %v is not positive", dpr)
	}
	if dpr == l.dpr {
		return nil
	}
...
func (l *Live) Draw() error {
	if l.b == nil {
		return errors.New("figure: Draw on a closed Live")
	}
	l.idx.Reset()
	l.next.Reset()
...
func (l *Live) Wheel(x, y, factor float64) error {
	if factor <= 0 {
		return fmt.Errorf("figure: zoom factor %v is not positive", factor)
	}
	pt := ir.Point{X: float32(x), Y: float32(y)}
	p, ok := l.panelAt(pt)
	if !ok {
...
// plot. figure does not remember which rows are selected, because a library
...
// placed to answer — with the column in hand, and without figure guessing
