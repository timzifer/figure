...
func livePlot(t *testing.T) (*figure.Plot, *figure.Live, *irtest.Recorder) {
	t.Helper()
...
func TestLiveNeedsATargetAndSomethingToDraw(t *testing.T) {
	if _, err := figure.New().Live(nil); err == nil {
		t.Error("a nil target was accepted")
	}
	if _, err := figure.New().Live(irtest.New().Target()); !errors.Is(err, figure.ErrNoLayers) {
...
func TestClickReportsWhatIsUnderIt(t *testing.T) {
	p, live, _ := livePlot(t)
	var seen []figure.Event
	p.On(figure.Click, func(ev figure.Event) { seen = append(seen, ev) })
...
func TestLeaveFiresOnce(t *testing.T) {
	p, live, _ := livePlot(t)
	var leaves int
	p.On(figure.Leave, func(figure.Event) { leaves++ })

	panel := live.Index().Panels()[0]
	live.Move(float64(panel.X.Map(1)), float64(panel.Y.Map(4)))
	if got := live.Leave(); got.Kind != figure.Leave {
...
func TestZoomToARectangle(t *testing.T) {
	p, live, _ := livePlot(t)
	var zooms int
	p.On(figure.Zoom, func(ev figure.Event) {
		zooms++
		if ev.Rect.Empty() {
...
func TestAnUnregisteredHandlerKindIsIgnored(t *testing.T) {
	p := figure.New(figure.Size(200, 200))
	p.Add(geom.Scatter(points(), geom.X("x"), geom.Y("y")))
	p.On(figure.Hover, nil) // a nil handler is dropped rather than panicking
...
func TestParseJSONRejectsRubbish(t *testing.T) {
	if _, err := figure.ParseJSON([]byte("not json")); err == nil {
		t.Error("invalid JSON was accepted")
	}
	if _, err := figure.ParseJSON([]byte(`{"layer":[{"mark":{"type":"violin"}}]}`)); err == nil {
