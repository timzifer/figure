...
func overlayLive(t *testing.T, o figure.Overlay) (*figure.Live, *irtest.Recorder) {
	t.Helper()
...
func middleOf(t *testing.T, l *figure.Live) ir.Point {
	t.Helper()
...
func TestNoOverlayChangesNothing(t *testing.T) {
	...
	_, empty := overlayLive(t, &figure.Crosshair{})
...
func TestAnOverlayIsNotHitTestable(t *testing.T) {
	...
	at := middleOf(t, bare)
	cross := &figure.Crosshair{At: at, Show: true, Panel: -1}
...
func TestACrosshairDrawsTwoRules(t *testing.T) {
	...
	_, rec := overlayLive(t, &figure.Crosshair{At: at, Show: true, Panel: -1})
	...
	_, one := overlayLive(t, &figure.Crosshair{At: at, Show: true, Panel: -1, NoHorizontal: true})
...
func TestACrosshairOutsideEveryPanelDrawsNothing(t *testing.T) {
	_, plain := overlayLive(t, nil)
	_, rec := overlayLive(t, &figure.Crosshair{
...
func TestHighlightRingsItsPoints(t *testing.T) {
	bare, plain := overlayLive(t, nil)
	at := middleOf(t, bare)
	_, rec := overlayLive(t, &figure.Highlight{
...
func TestBrushFillsAndOutlines(t *testing.T) {
	...
	_, rec := overlayLive(t, &figure.Brush{Rect: ir.Rect{
	...
	_, none := overlayLive(t, &figure.Brush{})
...
func TestTooltipDrawsABoxAndItsLines(t *testing.T) {
	bare, plain := overlayLive(t, nil)
	at := middleOf(t, bare)
	_, rec := overlayLive(t, &figure.Tooltip{
	...
	_, none := overlayLive(t, &figure.Tooltip{At: at})
...
func TestATooltipFlipsAtTheEdge(t *testing.T) {
	...
	_, left := overlayLive(t, &figure.Tooltip{At: near, Lines: []string{"hello"}})

	far := ir.Point{X: a.Max.X - 2, Y: a.Max.Y - 2}
	_, right := overlayLive(t, &figure.Tooltip{At: far, Lines: []string{"hello"}})
...
func TestMovingAnOverlayIsAPartialRepaint(t *testing.T) {
	cross := &figure.Crosshair{Show: true, Panel: -1}
...
func TestOverlaysDrawInOrder(t *testing.T) {
	bare, plain := overlayLive(t, nil)
	at := middleOf(t, bare)
	_, rec := overlayLive(t, figure.Overlays{
		&figure.Crosshair{At: at, Show: true, Panel: -1},
		nil, // skipped, so a caller may keep a fixed-length list
		&figure.Tooltip{At: at, Lines: []string{"one"}},
	})
	if got, want := rec.Count("Polyline"), plain.Count("Polyline")+2; got != want {
...
func TestAnOverlaySurvivesARebuild(t *testing.T) {
	...
	if live.CurrentOverlay() != figure.Overlay(cross) {
...
func TestAPlotCanCarryAnOverlay(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1}, "y": {1, 2}})
	...
	p.Overlay(&figure.Brush{Rect: ir.Rect{
...
func TestOverlayGolden(t *testing.T) {
	...
	p.Overlay(figure.Overlays{
		&figure.Brush{Rect: ir.Rect{
			Min: ir.Point{X: panel.X.Map(0.6), Y: panel.Y.Map(26)},
			Max: ir.Point{X: panel.X.Map(1.4), Y: panel.Y.Map(8)},
		}},
		&figure.Crosshair{At: at, Show: true, Panel: -1},
		&figure.Highlight{At: []ir.Point{at}, Panel: -1},
		&figure.Tooltip{At: at, Lines: []string{"x: 2", "y: 15"}},
	})
	golden(t, "overlays", p)
}
