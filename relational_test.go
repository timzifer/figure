...
func relationalPlot(size float32, c coord.Coord, layer geom.Geom) *figure.Plot {
	p := figure.New(
		figure.Size(int(size), int(size)),
		figure.Theme(bareRelational()),
		figure.Coord(c),
	)
...
func TestGoldenTreemap(t *testing.T) {
	p := figure.New(figure.Size(640, 400), figure.Theme(bareRelational()), figure.Title("Disk by directory"))
	p.X(scale.Linear())
	p.Y(scale.Linear())
...
func TestGoldenSankey(t *testing.T) {
	p := figure.New(figure.Size(640, 380), figure.Theme(bareRelational()), figure.Title("Requests per second"))
	p.X(scale.Linear())
	p.Y(scale.Linear())
...
func drawRelational(t *testing.T, p *figure.Plot) []irtest.Call {
	t.Helper()
	rec := irtest.New()
