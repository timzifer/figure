...
// questions. What it does ask, figure answered in English until now:
...
// because they are the ones figure could get right, not because they are the
...
// way to add one rather than a workaround. Shipping a table figure cannot
...
func RegisterLocale(l *Locale) {
	if l == nil || l.Name == "" {
		panic("figure/scale: RegisterLocale: a locale needs a name")
	}
	localeMu.Lock()
...
// written, and the tags figure ships are lower-case language codes.
...
// place: [github.com/timzifer/figure.Locale] walks the scales a plot holds
