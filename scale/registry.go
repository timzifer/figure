...
func Register(k Kind, build func(Desc) (Scale, error)) {
	if k == "" || build == nil {
		panic("figure/scale: Register: a kind needs a name and a builder")
	}
	if builtinKind(k) {
		panic(fmt.Sprintf("figure/scale: Register: %q is a built-in scale kind", k))
	}
	registryMu.Lock()
...
func RegisterColor(k ColorKind, build func(ColorDesc) (ColorScale, error)) {
	if k == "" || build == nil {
		panic("figure/scale: RegisterColor: a kind needs a name and a builder")
	}
	if builtinColorKind(k) {
		panic(fmt.Sprintf("figure/scale: RegisterColor: %q is a built-in colour scale kind", k))
	}
	registryMu.Lock()
	defer registryMu.Unlock()
