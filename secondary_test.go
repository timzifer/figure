...
func twoUnits(opts ...figure.Option) (*figure.Plot, scale.Scale, scale.Scale) {
	...
	p := figure.New(append([]figure.Option{
		figure.Size(600, 360), figure.YTitle("revenue"), figure.Y2Title("margin"),
	}, opts...)...)
...
func TestTheSecondaryAxisDrawsNoGrid(t *testing.T) {
	one := figure.New(figure.Size(600, 360))
	one.X(scale.Linear(scale.Nice()))
	one.Y(scale.Linear(scale.Nice(), scale.Zero()))
	src := figure.Float64Columns(map[string][]float64{"m": {0, 1, 2, 3}, "revenue": {1000, 1400, 1200, 1600}})
...
func gridLines(t *testing.T, p *figure.Plot) int {
	t.Helper()
	rec := irtest.New()
	if err := p.Render(rec.Target()); err != nil {
		t.Fatal(err)
...
func TestAPolarChartDrawsNoSecondAxis(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 0, 0}, "y": {1, 2, 3}})
	p := figure.New(figure.Size(400, 400), figure.Coord(coord.Pie()))
	p.X(scale.Linear())
	p.Y(scale.Linear())
...
func TestAChartWithNoSecondAxisIsUnchanged(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1, 2}, "y": {1, 4, 2}})
	build := func(withY2 bool) string {
		p := figure.New(figure.Size(500, 300), figure.Title("Signal"))
		p.X(scale.Linear(scale.Nice()))
...
func TestAFacetWritesTheSecondAxisAtItsRightEdge(t *testing.T) {
	...
	p := figure.New(figure.Size(700, 320))
	p.X(scale.Linear(scale.Nice()))
	p.Y(scale.Linear(scale.Nice()))
...
func twoExtents(opts ...figure.Option) (*figure.Plot, scale.Scale, scale.Scale) {
	src := figure.Float64Columns(map[string][]float64{
		"nm":     {400, 500, 600, 700},
	...
	p := figure.New(append([]figure.Option{
		figure.Size(640, 380),
		figure.XTitle("wavelength (nm)"), figure.X2Title("wavenumber (1/cm)"),
	}, opts...)...)
	p.X(x)
	p.X2(x2)
...
func TestTheSecondHorizontalAxisDrawsNoGrid(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"nm": {400, 700}, "i": {0.2, 0.3}})
	one := figure.New(figure.Size(640, 380))
	one.X(scale.Linear(scale.Nice()))
	one.Y(scale.Linear(scale.Nice(), scale.Zero()))
...
func verticalGridLines(t *testing.T, p *figure.Plot) int {
	t.Helper()
	rec := irtest.New()
	if err := p.Render(rec.Target()); err != nil {
		t.Fatal(err)
...
func TestALayerCanBeOnBothSecondaryAxes(t *testing.T) {
	...
	p := figure.New(figure.Size(600, 400))
	p.X(x)
	p.Y(y)
	p.X2(x2)
	p.Y2(y2)
...
func TestAFacetWritesTheSecondHorizontalAxisAtItsTop(t *testing.T) {
	...
	p := figure.New(figure.Size(420, 620))
	p.X(scale.Linear(scale.Nice()))
