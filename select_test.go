...
func selectLive(t *testing.T, layers ...geom.Geom) *figure.Live {
	t.Helper()
...
func boxAround(t *testing.T, l *figure.Live, layer int, rows ...int) ir.Rect {
	t.Helper()
	var r ir.Rect
...
func TestSelectFiresTheHandler(t *testing.T) {
	...
	p.On(figure.Select, func(ev figure.Event) { got = append(got, ev) })
...
func TestDragSelectsDoesNotPan(t *testing.T) {
	...
	in := live.Input().Drag(figure.DragSelects)
...
func TestDragZoomsToTheBand(t *testing.T) {
	...
	area := live.Index().Panels()[0].Area
	in := live.Input().Drag(figure.DragZooms)
