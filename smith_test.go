...
func smithGrid(p *figure.Plot) *figure.Plot {
...
func seriesRL(r float64, n int) figure.Source {
	rs, xs := make([]float64, n), make([]float64, n)
	for i := range n {
		rs[i] = r
		xs[i] = -2 + 4*float64(i)/float64(n-1)
	}
	return figure.Float64Columns(map[string][]float64{"r": rs, "x": xs})
}

func smithPlot(opts ...figure.Option) *figure.Plot {
...
func TestASmithChartIsALineInADifferentCoordinateSystem(t *testing.T) {
	...
	cartesian := figure.New(figure.Size(480, 480), figure.Legend(false))
	...
	if err := cartesian.Render(figure.SVGWriter(&flat)); err != nil {
		t.Fatal(err)
	}
	if err := smith.Render(figure.SVGWriter(&curved)); err != nil {
		t.Fatal(err)
	}
...
func TestOneLoadLandsInOnePlaceOnEitherChart(t *testing.T) {
	...
	y := smithGrid(figure.New(figure.Size(480, 480),
...
func placedAt(t *testing.T, p *figure.Plot, r, x float64) ir.Point {
	t.Helper()
	rec := irtest.New()
	live, err := p.Live(rec.Target())
	if err != nil {
...
func TestGoldenSmithChart(t *testing.T) {
	p := smithPlot(
		figure.Size(560, 520),
		figure.Title("A series RL load, swept"),
	)
