...
func (s Spec) Chart() (Chart, error) {
	...
	if s.Config != nil {
		if s.Config.Theme != "" {
			...
			if !ok {
				return Chart{}, fmt.Errorf("figure/spec: unknown theme %q", s.Config.Theme)
			}
			c.Theme = t
		}
	...
	if s.Encoding != nil {
		if c.X, c.XTitle, err = axisScale(s.Encoding.X); err != nil {
			return Chart{}, fmt.Errorf("figure/spec: x axis: %w", err)
		}
		if c.Y, c.YTitle, err = axisScale(s.Encoding.Y); err != nil {
			return Chart{}, fmt.Errorf("figure/spec: y axis: %w", err)
		}
		if c.Y2, c.Y2Title, err = axisScale(s.Encoding.YSecondary); err != nil {
			return Chart{}, fmt.Errorf("figure/spec: secondary y axis: %w", err)
		}
		if c.X2, c.X2Title, err = axisScale(s.Encoding.XSecondary); err != nil {
			return Chart{}, fmt.Errorf("figure/spec: secondary x axis: %w", err)
		}
	}
	...
	for i, l := range s.Layer {
		...
		if err != nil {
			return Chart{}, fmt.Errorf("figure/spec: layer %d: %w", i, err)
		}
	...
	for i, d := range s.Tracks {
		...
		if err != nil {
			return Chart{}, fmt.Errorf("figure/spec: track %d: %w", i, err)
		}
...
func decodeLayer(l Layer, shared data.Source) (geom.Geom, error) {
	...
	d := geom.Desc{
		...
		Callout:      l.Mark.Callout,
		MinFontSize:  l.Mark.MinFontSize,
		Wrap:         l.Mark.Wrap,
		Pinned:       l.Mark.Slide != nil && !*l.Mark.Slide,
	}
	...
	if s != nil {
		if len(s.Domain) == 2 {
			...
			if math.IsNaN(lo) || math.IsNaN(hi) {
				return nil, fmt.Errorf("figure/spec: a size domain needs two numbers")
			}
			d.Min, d.Max, d.Fixed = lo, hi, true
		} else if len(s.Domain) != 0 {
			return nil, fmt.Errorf("figure/spec: a size domain needs two bounds, got %d", len(s.Domain))
		}
		if len(s.SizeRange) == 2 {
			d.MinSize, d.MaxSize, d.RangeSet = s.SizeRange[0], s.SizeRange[1], true
		} else if len(s.SizeRange) != 0 {
			return nil, fmt.Errorf("figure/spec: a size range needs two diameters, got %d", len(s.SizeRange))
		}
		if s.SizeZero != nil {
...
func decodeColorScale(s Scale) (scale.ColorScale, error) {
	...
	if d.Kind == scale.KindNamed {
		...
		for _, v := range s.Domain {
			label, ok := v.(string)
			if !ok {
				return nil, fmt.Errorf("figure/spec: a named colour scale's domain holds category names")
			}
	...
	if len(s.Domain) == 2 {
		lo, hi := asNumber(s.Domain[0]), asNumber(s.Domain[1])
		if math.IsNaN(lo) || math.IsNaN(hi) {
			return nil, fmt.Errorf("figure/spec: a colour domain needs two numbers")
		}
		d.Min, d.Max, d.Fixed = lo, hi, true
	}
	return scale.ColorFromDesc(d)
