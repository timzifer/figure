...
// them, at the same level, so that a document reads the same whether figure
...
func (m Mark) MarshalJSON() ([]byte, error) {
	...
	for _, k := range keys {
		if markFields()[k] {
			return nil, fmt.Errorf("figure/spec: mark property %q is one this package defines", k)
		}
		v, err := json.Marshal(m.Extra[k])
		if err != nil {
			return nil, fmt.Errorf("figure/spec: mark property %q: %w", k, err)
		}
		obj[k] = v
	}
	...
	*m = Mark(p)
	...
	for k, raw := range obj {
		...
		if err := json.Unmarshal(raw, &v); err != nil {
			return fmt.Errorf("figure/spec: mark property %q: %w", k, err)
		}
		if m.Extra == nil {
