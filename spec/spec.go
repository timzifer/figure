...
// person who knows that vocabulary can read a figure spec without a manual.
// It is not a Vega-Lite subset, and it does not claim to be one — figure has
// marks Vega-Lite has no name for and Vega-Lite has transforms figure does
...
// other, and a document written by a v0.x figure reads too. The version in
...
// Vega-Lite spells them. The properties below it are figure's own: they name
...
//
// It is figure's own field, named plainly rather than smuggled through a
...
// figure cannot rebuild — so a pie here is `mark: "bar"` with
...
func Parse(b []byte) (Spec, error) {
	...
	if err := json.Unmarshal(b, &s); err != nil {
		return Spec{}, fmt.Errorf("figure/spec: %w", err)
	}
	return s, nil
}
