...
// The vocabulary: the translation between figure's names for things and the
...
func markType(m geom.Mark) (typ, orient string, err error) {
	...
	case geom.MarkHistogram:
		...
		// the channel. figure bins in the layer, so the mark carries the name
...
func geomMark(m Mark, enc *Encoding) (geom.Mark, error) {
	...
	if m.Type == "" {
		return "", fmt.Errorf("figure/spec: a mark needs a type")
	}
	// Anything else is a mark figure did not define. geom.FromDesc knows
...
// the next row, which is figure's default and what a sampled signal did.
var steps = []struct {
	pos  geom.StepPos
	name string
}{
...
func parseColor(s string) (ir.Color, error) {
	...
	case 6, 8:
	default:
		return ir.Color{}, fmt.Errorf("figure/spec: %q is not a hex colour", s)
	}
