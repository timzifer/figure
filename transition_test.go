...
// source says.
func tweened(t *testing.T, opts ...data.TweenOption) (*figure.Live, *data.Tween, *irtest.Recorder) {
	t.Helper()
...
func frameAt(t *testing.T, live *figure.Live, rec *irtest.Recorder, tr *figure.Transition, f float64) []string {
	t.Helper()
	rec.Reset()
...
// The host owns the clock: figure never reads one, so a synthetic time drives
...
func TestAdvanceRunsOnTheHostsClock(t *testing.T) {
	...
	tr.Ease(figure.EaseLinear).Over(time.Second)
...
func TestRescaleSlidesTheAxes(t *testing.T) {
	...
	tr.Ease(figure.EaseLinear).Rescale(true)
...
func TestATransitionNeedsATween(t *testing.T) {
	live, _, _ := tweened(t)
	if _, err := live.Transition(); err != figure.ErrNoTweens {
		t.Errorf("err = %v, want ErrNoTweens", err)
	}
	if _, err := live.Transition(nil); err != figure.ErrNoTweens {
		t.Errorf("a nil tween gave %v, want ErrNoTweens", err)
	}
}

func TestEasingCurvesRunFromZeroToOne(t *testing.T) {
	for name, e := range map[string]figure.Easing{
		"linear": figure.EaseLinear,
		"in":     figure.EaseIn,
		"out":    figure.EaseOut,
		"inout":  figure.EaseInOut,
	} {
		if got := e(0); got != 0 {
...
func benchmarkTransitionFrame(b *testing.B, n int) {
	...
	a := figure.NewTable().String("id", ids).Float64("x", x).Float64("y", ya)
	end := figure.NewTable().String("id", ids).Float64("x", x).Float64("y", yb)
...
func TestALabelOverANumberCounts(t *testing.T) {
	a := figure.NewTable().String("id", []string{"a"}).
		Float64("x", []float64{1}).Float64("y", []float64{1}).
		Float64("n", []float64{0})
	b := figure.NewTable().String("id", []string{"a"}).
		Float64("x", []float64{1}).Float64("y", []float64{1}).
