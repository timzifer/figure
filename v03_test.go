...
import (
	...
	"github.com/timzifer/figure/backend/pdf"
...
func facetSource() figure.Source {
	var region []string
	var x, y []float64
	...
	return figure.NewTable().String("region", region).Float64("t", x).Float64("v", y)
}

func facetPlot() *figure.Plot {
	p := figure.New(
		figure.Size(760, 420),
		figure.Title("Throughput by region"),
		figure.XTitle("hour"),
		figure.YTitle("requests/s"),
	)
...
func TestGoldenColorbar(t *testing.T) {
	...
	p := figure.New(figure.Size(620, 380), figure.Title("Sales by region"))
	p.X(scale.Ordinal())
	p.Y(scale.Linear(scale.Nice(), scale.Zero()))
...
func TestGoldenSubplotGrid(t *testing.T) {
	xs := ramp(0, 10, 40)
	panel := func(title string, fn func(float64) float64, c int) *figure.Plot {
	...
	if err := g.Render(figure.SVGWriter(&buf, svg.Pretty())); err != nil {
...
func TestFacetErrorsSurface(t *testing.T) {
	err := facetPlot().Facet(facet.Wrap("nope")).Render(figure.SVGWriter(&bytes.Buffer{}))
...
func TestAFreeAxisNeedsACloneableScale(t *testing.T) {
	...
	err := p.Render(figure.SVGWriter(&bytes.Buffer{}))
...
func TestGridRendersEveryPlot(t *testing.T) {
	g := figure.NewGrid(2, figure.GridSize(600, 400))
	for _, name := range []string{"a", "b", "c"} {
		p := figure.New(figure.Title(name))
		p.Add(geom.Line(figure.Float64Columns(map[string][]float64{
			"x": {0, 1, 2}, "y": {0, 1, 0},
		}), geom.X("x"), geom.Y("y")))
		g.Add(p)
...
func TestGridAtPlacesAPlotAndLeavesHoles(t *testing.T) {
	p := figure.New(figure.Title("corner"))
	p.Add(geom.Line(figure.Float64Columns(map[string][]float64{
		"x": {0, 1}, "y": {0, 1},
	}), geom.X("x"), geom.Y("y")))

	g := figure.NewGrid(2, figure.GridSize(600, 400)).At(1, 1, p)
	var buf bytes.Buffer
	if err := g.Render(figure.SVGWriter(&buf)); err != nil {
		t.Fatal(err)
	}
...
func TestGridAtReplacesACell(t *testing.T) {
	first := figure.New(figure.Title("first"))
	second := figure.New(figure.Title("second"))
	for _, p := range []*figure.Plot{first, second} {
		p.Add(geom.Line(figure.Float64Columns(map[string][]float64{
			"x": {0, 1}, "y": {0, 1},
		}), geom.X("x"), geom.Y("y")))
	}
	g := figure.NewGrid(1, figure.GridSize(400, 300)).Add(first).At(0, 0, second)
	var buf bytes.Buffer
	if err := g.Render(figure.SVGWriter(&buf)); err != nil {
		t.Fatal(err)
	}
...
func TestEmptyGridIsAnError(t *testing.T) {
	err := figure.NewGrid(2).Render(figure.SVGWriter(&bytes.Buffer{}))
	if !errors.Is(err, figure.ErrEmptyGrid) {
		t.Errorf("err = %v, want ErrEmptyGrid", err)
	}
	if err := figure.NewGrid(2).Render(nil); err == nil {
...
func TestGridLegendGathersEveryPanel(t *testing.T) {
	g := figure.NewGrid(2, figure.GridSize(700, 500), figure.GridLegend(true))
...
func TestPDFRendersTheSameChart(t *testing.T) {
	...
	if err := facetPlot().Render(figure.PDF(path, pdf.Title("Throughput"))); err != nil {
		t.Fatal(err)
	}
