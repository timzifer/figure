...
func noisySignal(n int) figure.Source {
	...
	y[n/3] = 4
	return figure.Float64Columns(map[string][]float64{"x": x, "y": y})
}
...
func TestABigSeriesRendersAsASmallFile(t *testing.T) {
	...
	render := func(opts ...geom.Option) int {
		t.Helper()
		p := figure.New(figure.Size(800, 500), figure.Title("A million rows"))
...
// figure decides. What figure decides is that there is one image, covering
...
func gaussianCloud(n int) figure.Source {
...
func TestAGridRendersInParallel(t *testing.T) {
	g := figure.NewGrid(2, figure.GridSize(800, 600), figure.GridTitle("Fleet"))
	for range 4 {
		p := figure.New(figure.Title("panel"))
		p.Add(geom.Line(noisySignal(50_000), geom.X("x"), geom.Y("y")))
		g.Add(p)
	}
	var par, ser bytes.Buffer
	if err := g.Render(figure.SVGWriter(&par)); err != nil {
		t.Fatalf("Render: %v", err)
	}

	h := figure.NewGrid(2, figure.GridSize(800, 600), figure.GridTitle("Fleet"), figure.GridParallel(false))
	for range 4 {
		p := figure.New(figure.Title("panel"))
		p.Add(geom.Line(noisySignal(50_000), geom.X("x"), geom.Y("y")))
		h.Add(p)
	}
	if err := h.Render(figure.SVGWriter(&ser)); err != nil {
		t.Fatalf("Render: %v", err)
	}
