...
func chart() *figure.Plot { p, _ := chartOn(scale.Linear(scale.Nice())); return p }
...
func chartOn(x scale.Scale) (*figure.Plot, scale.Scale) {
	src := figure.NewTable().
...
func TestAChartCarriesItsNameIntoTheDocument(t *testing.T) {
	...
	if err := chart().Render(figure.SVGWriter(&buf)); err != nil {
		t.Fatal(err)
	}
	doc := buf.String()
	for _, want := range []string{`role="img"`, `aria-labelledby="figure-title"`, "<title id=\"figure-title\">Throughput</title>"} {
...
func TestADescribedChartCarriesTheDescriptionToo(t *testing.T) {
	...
	doc := buf.String()
	if !strings.Contains(doc, `aria-labelledby="figure-title figure-desc"`) {
		t.Error("the document does not point at both its title and its description")
	}
	if !strings.Contains(doc, "<desc id=\"figure-desc\">") {
...
func TestAnExplicitDescriptionWins(t *testing.T) {
	...
	p := figure.New(figure.Title("Throughput"), figure.Description("Fig. 3", "Rows per second over five batches."))
	...
	if got := buf.String(); !strings.Contains(got, "<title id=\"figure-title\">Fig. 3</title>") {
...
func TestALiveChartKeepsItsName(t *testing.T) {
	var buf bytes.Buffer
	live, err := chart().Live(figure.SVGWriter(&buf))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(buf.String(), "<title id=\"figure-title\">Throughput</title>") {
...
func chartInto(b ir.Backend, th theme.Theme) error {
	...
	p := figure.New(figure.Size(640, 400), figure.Theme(th))
	p.Add(
...
func TestRedundantEncodingLeavesAnExplicitChoiceAlone(t *testing.T) {
	...
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1}, "y": {0, 1}, "z": {1, 0}})
	p := figure.New(figure.Theme(th))
	p.Add(
		geom.Line(src, geom.X("x"), geom.Y("y")),
...
func TestAResponsiveChartScalesItsTypeWithItsSize(t *testing.T) {
	sizes := func(p *figure.Plot, w, h int) (float64, float32) {
		rec := irtest.New()
...
// on the chart above.
func responsive() *figure.Plot {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1, 2, 3, 4}, "y": {2, 5, 3, 9, 4}})
	p := figure.New(
		figure.Size(640, 400),
...
func TestALabelIsMeasuredAsItIsDrawn(t *testing.T) {
	...
	plain := yTitleX(t, figure.New(figure.Size(400, 300), figure.YTitle("sigma")))
	typeset := yTitleX(t, figure.New(
		figure.Size(400, 300),
		figure.Math(mathtext.TeX()),
		figure.YTitle(`$\sigma$`),
	))
...
func yTitleX(t *testing.T, p *figure.Plot) float64 {
	t.Helper()
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1}, "y": {0, 1}})
	p.Add(geom.Line(src, geom.X("x"), geom.Y("y")))
...
func TestAChartWithoutATypesetterLeavesLabelsAlone(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1}, "y": {0, 1}})
	p := figure.New(figure.Title(`$5 to $10`))
	p.Add(geom.Line(src, geom.X("x"), geom.Y("y")))
...
func trace(p *figure.Plot) (string, error) {
	rec := irtest.New()
	if err := p.Render(rec.Target()); err != nil {
		return "", err
	}
...
func TestOneStateMachineSteersEverySurface(t *testing.T) {
	p := chart()
	var kinds []figure.EventKind
	for _, k := range []figure.EventKind{figure.Hover, figure.Click, figure.Pan, figure.Zoom, figure.Leave} {
		p.On(k, func(ev figure.Event) { kinds = append(kinds, ev.Kind) })
	}
	...
	want := []figure.EventKind{
		figure.Hover, figure.Click, figure.Pan, figure.Zoom, figure.Leave,
	}
...
func TestADragDoesNotAlsoClick(t *testing.T) {
	p := chart()
	clicks := 0
	p.On(figure.Click, func(figure.Event) { clicks++ })
...
func thumbnail(w, h int, scaled bool) *figure.Plot {
	opts := []figure.Option{figure.Size(w, h), figure.Title("Throughput")}
	if scaled {
		opts = append(opts, figure.ResponsiveFrom(800, 500))
	}
	p := figure.New(opts...)
...
func TestAGridCarriesItsNameAndItsNotation(t *testing.T) {
	src := figure.Float64Columns(map[string][]float64{"x": {0, 1, 2}, "y": {1, 3, 2}})
	panel := func(title string) *figure.Plot {
	...
	g := figure.NewGrid(2,
		figure.GridSize(600, 400),
		figure.GridTitle("Fleet"),
		figure.GridMath(mathtext.TeX()),
		figure.GridDescription("Fleet overview", "Two panels, one per host."),
	)
	g.Add(panel(`host $\alpha$`), panel(`host $\beta$`))

	var buf bytes.Buffer
	if err := g.Render(figure.SVGWriter(&buf)); err != nil {
		t.Fatal(err)
	}
	doc := buf.String()
	for _, want := range []string{
		"<title id=\"figure-title\">Fleet overview</title>",
