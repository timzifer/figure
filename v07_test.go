...
func traffic() figure.Source {
	...
	return figure.NewTable().Float64("t", t).Float64("v", v).String("region", region)
}

func TestOneLayerDrawsEverySeriesAndTheLegendNamesThem(t *testing.T) {
	p := figure.New(figure.Size(600, 400), figure.Title("Traffic"))
...
func TestAHeatmapRendersFromThePublicAPI(t *testing.T) {
	...
	src := figure.NewTable().String("day", day).String("hour", hour).Float64("calls", calls)

	p := figure.New(figure.Size(500, 320), figure.Title("Calls"))
	p.X(scale.Ordinal())
	p.Y(scale.Ordinal())
	...
	if err := p.Render(figure.SVGWriter(&out)); err != nil {
		t.Fatalf("Render: %v", err)
	}
	doc := out.String()
...
func TestAGanttChartRendersFromThePublicAPI(t *testing.T) {
	...
	p := figure.New(figure.Size(600, 240), figure.Title("Plan"))
	p.X(scale.Time())
	p.Y(scale.Ordinal())
...
func TestGoldenStackedAndHeatmap(t *testing.T) {
	t.Run("stacked", func(t *testing.T) {
		p := figure.New(figure.Size(480, 320), figure.Title("Stacked"))
		p.X(scale.Linear())
	...
	t.Run("heatmap", func(t *testing.T) {
		...
		src := figure.NewTable().String("day", day).String("hour", hour).Float64("calls", calls)
		p := figure.New(figure.Size(480, 320), figure.Title("Heatmap"))
...
func TestAFacetedStackIsTheSameChartOnEveryGoroutine(t *testing.T) {
	...
	build := func(parallel bool) []string {
		p := figure.New(figure.Size(700, 460), figure.Parallel(parallel))
