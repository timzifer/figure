...
func donut(opts ...geom.Option) *figure.Plot {
	p := figure.New(figure.Size(400, 400), figure.Theme(bare()),
		figure.Coord(coord.Pie(coord.Radius(1))), figure.Legend(false))
...
func TestADonutIsTheRecipeItNames(t *testing.T) {
	draw := func(c coord.Coord) []string {
		t.Helper()
		p := figure.New(figure.Size(400, 400), figure.Theme(bare()),
			figure.Coord(c), figure.Legend(false))
		p.X(scale.Linear())
		p.Y(scale.Linear())
...
func TestABrokenOutSliceLeavesTheRingAndKeepsItsRow(t *testing.T) {
	probe := func(p *figure.Plot) (*interact.Index, func()) {
		t.Helper()
		rec := irtest.New()
