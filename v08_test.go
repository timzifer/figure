...
func pie(opts ...coord.PolarOption) *figure.Plot {
	p := figure.New(figure.Size(400, 400), figure.Theme(bare()),
		figure.Coord(coord.Polar(append([]coord.PolarOption{coord.Theta(coord.FromY)}, opts...)...)),
		figure.Legend(false))
...
func TestOneBarLayerDrawsABarChartAndAPie(t *testing.T) {
	...
	cart := figure.New(figure.Size(400, 400), figure.Legend(false))
	cart.X(scale.Linear())
	cart.Y(scale.Linear())
	cart.Add(layer())

	polar := figure.New(figure.Size(400, 400), figure.Theme(bare()), figure.Legend(false),
		figure.Coord(coord.Polar(coord.Theta(coord.FromY))))
...
func TestAHitInvertsThroughTheCoord(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 400), figure.Theme(bare()),
		figure.Coord(coord.Polar()), figure.Legend(false))
...
func TestARadarIsALineWithChordsThatCloses(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 400), figure.Coord(coord.Polar(coord.Chord())),
		figure.Legend(false))
...
func TestPolarFurnitureIsRingsAndLabelsRoundTheRing(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 400), figure.Coord(coord.Polar()), figure.Legend(false))
	p.X(scale.Ordinal(scale.OrdinalPadding(0)))
	p.Y(scale.Linear(scale.Domain(0, 10)))
	p.Add(geom.Line(src, geom.X("axis"), geom.Y("v"), geom.Closed(true)))
...
// through an `arc` mark figure could not rebuild. See ADR 0014.
func TestTheSpecRoundTripsTheCoord(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 400), figure.Legend(false),
		figure.Coord(coord.Polar(coord.Theta(coord.FromY), coord.Hole(0.4),
...
// Cartesian one draw the same thing, and a field in every document figure has
// ever written would be noise in every one of them.
func TestACartesianChartWritesNoCoord(t *testing.T) {
	p := figure.New(figure.Size(300, 200))
	p.Add(geom.Line(figure.Float64Columns(map[string][]float64{"x": {0, 1}, "y": {1, 2}}),
		geom.X("x"), geom.Y("y")))
	b, err := p.MarshalJSON()
	if err != nil {
		t.Fatal(err)
...
func TestAFacetsPanelsShareTheCoord(t *testing.T) {
	...
	src := figure.NewTable().String("region", region).Float64("all", one).Float64("share", share)

	p := figure.New(figure.Size(600, 320), figure.Theme(bare()), figure.Legend(false),
		figure.Coord(coord.Polar(coord.Theta(coord.FromY))))
	p.X(scale.Linear())
	p.Y(scale.Linear())
...
func TestAPolarLayerDoesNotDecimate(t *testing.T) {
	...
	count := func(c coord.Coord) int {
		p := figure.New(figure.Size(400, 400), figure.Coord(c), figure.Legend(false))
...
func TestZoomingAPolarChartMovesTheRadialDomain(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 400), figure.Theme(bare()), figure.Legend(false),
		figure.Coord(coord.Polar()))
	p.X(scale.Linear(scale.Domain(0, 4)))
	p.Y(scale.Linear())
	p.Add(geom.Scatter(src, geom.X("angle"), geom.Y("r")))
