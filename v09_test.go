...
func nations() *figure.Plot {
	...
	p := figure.New(figure.Size(900, 560), figure.Title("Nations"), figure.Legend(true))
...
func TestDoublingAValueMultipliesABubblesDiameterByRootTwo(t *testing.T) {
	...
	p := figure.New(figure.Size(600, 400), figure.Legend(false))
	p.X(scale.Linear(scale.Nice()))
	p.Y(scale.Linear(scale.Nice()))
...
func TestAViolinComposesWithGroupsAndFacets(t *testing.T) {
	...
	p := figure.New(figure.Size(900, 400), figure.Legend(true))
	p.X(scale.Ordinal())
	p.Y(scale.Linear(scale.Nice()))
...
func TestARidgelineNeedsACategoricalAxis(t *testing.T) {
	...
	ok := figure.New(figure.Size(600, 400), figure.Legend(false))
	...
	bad := figure.New(figure.Size(600, 400), figure.Legend(false))
	bad.X(scale.Linear())
	bad.Y(scale.Linear())
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			trace := func(parallel bool) []string {
				p := figure.New(figure.Size(700, 450), figure.Parallel(parallel))
				p.X(tc.x())
				p.Y(tc.y())
				p.Add(tc.layer())
...
func TestTheDistributionChartsRenderToSVG(t *testing.T) {
	...
	if err := nations().Render(figure.SVGWriter(&b)); err != nil {
		t.Fatal(err)
	}
