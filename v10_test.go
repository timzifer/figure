...
func machine(opts ...figure.Option) (*figure.Plot, *figure.Track) {
	speed := figure.Float64Columns(map[string][]float64{
		"t":     {0, 1, 2, 3, 4, 5, 6, 7},
		"speed": {120, 118, 121, 40, 38, 119, 122, 120},
	})
	states := figure.NewTable().
	...
	tr := p.Track(figure.Bottom, figure.TrackSize(48))
...
func panelsOf(t *testing.T, p *figure.Plot) ([]interact.Panel, *interact.Index) {
	t.Helper()
	rec := irtest.New()
...
func TestATrackDoesNotMoveThePanelsYDomain(t *testing.T) {
	plain := figure.New(figure.Size(640, 400))
	plain.X(scale.Linear(scale.Nice())).Y(scale.Linear(scale.Nice()))
	plain.Add(geom.Line(figure.Float64Columns(map[string][]float64{
		"t":     {0, 1, 2, 3, 4, 5, 6, 7},
		"speed": {120, 118, 121, 40, 38, 119, 122, 120},
	}), geom.X("t"), geom.Y("speed")))

	withTrack, _ := machine(figure.Size(640, 400))
...
func plainY(t *testing.T, p *figure.Plot) (float64, float64) {
	t.Helper()
...
func TestATrackKeepsItsHeightAtEveryCanvasSize(t *testing.T) {
	for _, size := range []struct{ w, h int }{{400, 300}, {640, 400}, {1600, 900}} {
		p, _ := machine(figure.Size(size.w, size.h))
		panels, _ := panelsOf(t, p)
		if len(panels) != 2 {
...
func TestATrackTakesItsHeightOutOfThePanel(t *testing.T) {
	...
	p, _ := machine(figure.Size(640, 400))
	panels, _ := panelsOf(t, p)
...
func TestAFractionalTrackScalesWithTheCanvas(t *testing.T) {
	...
	for _, h := range []int{400, 800} {
		p := figure.New(figure.Size(640, h))
		p.X(scale.Linear(scale.Nice())).Y(scale.Linear(scale.Nice()))
		p.Add(geom.Line(figure.Float64Columns(map[string][]float64{
			"t": {0, 1}, "speed": {1, 2},
		}), geom.X("t"), geom.Y("speed")))
		p.Track(figure.Bottom, figure.TrackFraction(0.1)).
			Add(geom.Line(figure.Float64Columns(map[string][]float64{
				"t": {0, 1}, "lane": {0, 1},
...
func TestATrackAndAFacetAreAnError(t *testing.T) {
	...
	if err := p.Render(figure.SVGWriter(discard{})); err != figure.ErrTrackWithFacet {
...
func TestGoldenTrack(t *testing.T) {
	p, _ := machine(figure.Size(640, 400), figure.Title("Line 3"), figure.YTitle("m/min"))
	golden(t, "track", p)
}
...
func TestStackedPlotsSharingAScaleShareTheirAxis(t *testing.T) {
	...
	speed.Add(geom.Line(figure.Float64Columns(map[string][]float64{
		"t": {0, 4, 8}, "speed": {120, 40, 122},
	}), geom.X("t"), geom.Y("speed")))

	states := figure.New()
	states.X(x).Y(scale.Ordinal())
	states.Add(geom.Rect(figure.NewTable().
	...
	g := figure.NewGrid(1,
		figure.GridSize(640, 400),
		figure.GridRowHeights(0, 48),
...
func TestATrackSurvivesTheRoundTrip(t *testing.T) {
	p, _ := machine(figure.Size(640, 400), figure.Title("Line 3"))

	b, err := p.MarshalJSON()
	if err != nil {
		t.Fatalf("MarshalJSON: %v", err)
	}
	q, err := figure.ParseJSON(b)
	if err != nil {
	...
	if got := tracks[0].Edge(); got != figure.Bottom {
...
func TestATrackedChartIsTheSameDrawnInParallel(t *testing.T) {
	build := func(serial, besideToo bool) []byte {
		t.Helper()
		opts := []figure.Option{figure.Size(700, 460), figure.Title("Line 3")}
		if serial {
			opts = append(opts, figure.Parallel(false))
		}
		p, _ := machine(opts...)
		if besideToo {
			p.Track(figure.Left, figure.TrackSize(56)).
				Add(geom.Rect(figure.NewTable().
...
func marginal(edge figure.Edge, opts ...figure.Option) *figure.Plot {
	speed := figure.Float64Columns(map[string][]float64{
		"t":     {0, 1, 2, 3, 4, 5, 6, 7},
		"speed": {120, 118, 121, 40, 38, 119, 122, 120},
	})
	bands := figure.NewTable().
		Float64("lo", []float64{38, 100}).
		Float64("hi", []float64{100, 122}).
		String("band", []string{"slow", "nominal"})
	...
	p.Track(edge, figure.TrackSize(56)).
		Add(geom.Rect(bands,
...
func TestALeftTrackTakesItsWidthFromThePanel(t *testing.T) {
	for _, size := range []struct{ w, h int }{{400, 300}, {640, 400}, {1600, 900}} {
		p := marginal(figure.Left, figure.Size(size.w, size.h))
		panels, _ := panelsOf(t, p)
		if len(panels) != 2 {
			t.Fatalf("%dx%d: %d panels, want 2", size.w, size.h, len(panels))
		}
...
func TestARightTrackSitsOnTheOtherSide(t *testing.T) {
	p := marginal(figure.Right, figure.Size(640, 400))
	panels, _ := panelsOf(t, p)
	panel, track := panels[0].Area, panels[1].Area
...
func TestAVerticalTrackDoesNotMoveThePanelsXDomain(t *testing.T) {
	...
	bare, _ := panelsOf(t, plain)
	withTrack, _ := panelsOf(t, marginal(figure.Left, figure.Size(640, 400)))
...
func TestAZoomOnYMovesThePanelAndAVerticalTrackTogether(t *testing.T) {
	p := marginal(figure.Left, figure.Size(640, 400))
	rec := irtest.New()
	live, err := p.Live(rec.Target())
	if err != nil {
		t.Fatalf("Live: %v", err)
...
func TestTracksOnTwoEdgesLeaveTheCornerEmpty(t *testing.T) {
	p, _ := machine(figure.Size(700, 460))
	p.Track(figure.Left, figure.TrackSize(56)).
		Add(geom.Rect(figure.NewTable().
			Float64("lo", []float64{38, 100}).
			Float64("hi", []float64{100, 122}).
...
func TestGoldenTrackBesideAndBelow(t *testing.T) {
	p, _ := machine(figure.Size(700, 460), figure.Title("Line 3"), figure.YTitle("m/min"))
	p.Track(figure.Left, figure.TrackSize(56), figure.TrackAxis(false)).
		Add(geom.Rect(figure.NewTable().
			Float64("lo", []float64{38, 100}).
			Float64("hi", []float64{100, 122}).
			String("band", []string{"slow", "nominal"}),
...
func TestATrackTrainsTheAxisItShares(t *testing.T) {
	p := figure.New(figure.Size(640, 400))
	...
	p.Track(figure.Bottom, figure.TrackSize(24)).
		Add(geom.Rect(figure.NewTable().
