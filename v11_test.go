...
import (
	...
	"github.com/timzifer/figure/internal/sfnttest"
	"github.com/timzifer/figure/scale"
)

// svgOf renders a plot and hands back the document.
func svgOf(t *testing.T, p *figure.Plot) string {
	t.Helper()
...
func TestALocaleReachesEveryAxisOfAPlot(t *testing.T) {
	...
	p := figure.New(figure.Size(500, 300), figure.Locale(scale.LocaleDE))
...
func TestALocaleReachesATracksOwnAxis(t *testing.T) {
	...
	load := figure.Float64Columns(map[string][]float64{
		"x": {0, 1, 2, 3},
	...
	p.Add(geom.Line(src, geom.X("x"), geom.Y("y")))
	tr := p.Track(figure.Bottom,
		figure.TrackScale(scale.Linear(scale.Nice(), scale.NumberFormat("#,"))),
		figure.TrackAxis(true))
...
func TestALocaleReachesAFreeFacetAxis(t *testing.T) {
	...
	p := figure.New(figure.Size(600, 400), figure.Locale(scale.LocaleDE))
	p.X(scale.Linear(scale.Nice()))
...
func TestALocaleNamesTheMonthsOfATimeAxis(t *testing.T) {
	...
	p := figure.New(figure.Size(600, 300), figure.Locale(scale.LocaleDE))
...
func TestAChartWithNoLocaleIsUnchanged(t *testing.T) {
	...
	build := func(opts ...figure.Option) string {
	...
	if build() != build(figure.Locale(scale.English)) {
...
func TestAnErrorBarRendersOverTheBarsItAnnotates(t *testing.T) {
	...
	p := figure.New(figure.Size(500, 300), figure.Title("Means"))
	p.X(scale.Ordinal())
...
func TestAPDFCarriesTheFontItsLabelsNeed(t *testing.T) {
	...
	p := figure.New(figure.Size(400, 300), figure.Locale(scale.LocaleDE))
	p.X(scale.Ordinal())
	p.Y(scale.Linear(scale.Nice(), scale.NumberFormat("#,.1")))
	...
	var buf bytes.Buffer
	if err := p.Render(figure.PDFWriter(&buf, pdf.Uncompressed(), pdf.WithFont(font, nil, nil))); err != nil {
		t.Fatalf("Render: %v", err)
	}
	doc := buf.String()
