...
func twoSeries(t *testing.T) (*figure.Plot, *figure.Live, *irtest.Recorder) {
	t.Helper()
	...
	p := figure.New(figure.Size(640, 320), figure.Legend(true))
...
// pointer would.
func rowOf(t *testing.T, l *figure.Live, label string) ir.Point {
	t.Helper()
	ix := l.Index()
	for x := float32(2); x < 640; x += 2 {
		for y := float32(2); y < 320; y += 2 {
			hit, ok := ix.At(ir.Point{X: x, Y: y}, 0)
			if ok && hit.Kind == figure.LegendRow && hit.Series == label {
...
func TestAHoverInTheMarginFindsTheLegend(t *testing.T) {
	...
	p.On(figure.Leave, func(ev figure.Event) { got = ev })

	at := rowOf(t, live, "a")
	live.Move(float64(at.X), float64(at.Y))
	if !got.Found || got.Hit.Kind != figure.LegendRow {
...
func TestAHiddenSeriesKeepsItsLegendRow(t *testing.T) {
	...
	at := rowOf(t, live, "b")
	hit, ok := live.Index().At(at, 0)
	if !ok || hit.Kind != figure.LegendRow {
...
func TestHiddenSeriesGolden(t *testing.T) {
	...
	p := figure.New(
		figure.Size(640, 320),
		figure.Legend(true),
		figure.Title("One series put away"),
		figure.XTitle("x"),
		figure.YTitle("y"),
	)
